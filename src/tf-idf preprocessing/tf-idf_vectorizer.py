...
for idx in df_dataset_1[df_dataset_1['plaintext'].isnull()].index:
    if df_dataset_1.loc[idx, 'label'] == 1:
        ...
        df_dataset_1.loc[idx, 'plaintext'] = "Hi, just checking in to see if you are available for our meeting tomorrow."
...
for min_df in range(5, 11):

    for max_df in [round(x, 2) for x in list(frange(0.80, 0.951, 0.01))]:

        vectorizer = TfidfVectorizer(
            lowercase=True,
            stop_words='english',           # get rid of popular nonsense eng words
            max_features=5000,              # limit of characterize tokens
            ngram_range=(1, 2),             # unigram + bigram 
            min_df=min_df,                  # discard tokens too rare (appear on less than [min_df] docs)
            max_df=max_df,                  # discard tokens too popular (appear on > [max_df]% documents)
            sublinear_tf=True               # apply log(1 + tf)
        )


        # fit and transform plaintext data (both traning and testing dataset) into TF-IDF matrix
        X_1 = vectorizer.fit_transform(df_dataset_1['plaintext'])
        X_2 = vectorizer.fit_transform(df_dataset_2['plaintext'])
        X_test  = vectorizer.transform(df_testing_dataset['plaintext'])

        '''
        tfidf_means = X_1.mean(axis=0).A1
        terms = vectorizer.get_feature_names_out()
        df_temp = pd.DataFrame({'term': terms, 'mean_tfidf': tfidf_means})
        lowest_10 = df_temp.sort_values(by='mean_tfidf').head(10)
        print(lowest_10)
        exit(0)
        '''
        ...
        isoForest_dataset_1 = IsolationForest(
            n_estimators=100,
            max_samples=256,
            contamination= 7500 / 17500,  # Dataset 1 has 7500 spam email
            random_state=42
        )
        ...
        isoForest_dataset_2 = IsolationForest(
            n_estimators=100,
            max_samples=256,
            contamination= 5000 / 17500,  # Dataset 2 has 5000 spam email
            random_state=42
        )
        ...
        predicts_dataset_1 = isoForest_dataset_1.predict(X_test)
        scores_dataset_2 = isoForest_dataset_2.decision_function(X_test)
        predicts_dataset_2 = isoForest_dataset_2.predict(X_test)

        preds_1 = (predicts_dataset_1 == -1).astype(int)
        preds_2 = (predicts_dataset_2 == -1).astype(int)

        report_1 = classification_report(y_test, preds_1, output_dict=True)
        report_2 = classification_report(y_test, preds_2, output_dict=True)

        # we select f1_score since 'spam' is the positive class in the problem,
        # and f1_score have the balance between precision and recall
        f1_spam_report_1 = report_1['1.0']['f1-score']
        f1_spam_report_2 = report_2['1.0']['f1-score']

        optimizing_result += f"\nCurrent config for dataset 1 : min_df={min_df}, max_df={max_df}, F1 for detect spam = {f1_spam_report_1:.4f}"
        if f1_spam_report_1 > best_score_dataset_1:
            best_score_dataset_1 = f1_spam_report_1
            best_config_dataset_1 = {
                'X_1_best': X_1,
                'X_test_dataset_1': X_test
            }

            optimizing_result += f"\nNew best config for dataset 1 found: min_df={min_df}, max_df={max_df}, F1 for detect spam = {f1_spam_report_1:.4f}"


        optimizing_result += f"\nCurrent config for dataset 2 : min_df={min_df}, max_df={max_df}, F1 for detect spam = {f1_spam_report_2:.4f}"
        if f1_spam_report_2 > best_score_dataset_2:
            best_score_dataset_2 = f1_spam_report_2
            best_config_dataset_2 = {
                'X_2_best': X_2,
                'X_test_dataset_2': X_test
            }

            optimizing_result += f"\nNew best config for dataset 2 found: min_df={min_df}, max_df={max_df}, F1 for detect spam = {f1_spam_report_2:.4f}"
